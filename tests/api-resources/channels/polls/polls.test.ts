...
describe('resource polls', () => {
  test('expire', async () => {
    const responsePromise = client.channels.polls.expire('891', '891');
  ...
  test('expire: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.channels.polls.expire('891', '891', { path: '/_stainless_unknown_path' }))
      .rejects
