...
describe('resource entitlements', () => {
  test('create: only required params', async () => {
    const responsePromise = client.applications.entitlements.create('891', {
      owner_id: '891',
  ...
  test('create: required and optional params', async () => {
    const response = await client.applications.entitlements.create('891', {
      owner_id: '891',
      owner_type: undefined,
      sku_id: '891',
    });
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(
      client.applications.entitlements.retrieve('891', '891', { path: '/_stainless_unknown_path' }),
  ...
  test('list: required and optional params', async () => {
    const response = await client.applications.entitlements.list('891', {
      sku_ids: 'string',
      after: '891',
      before: '891',
  ...
  test('delete: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.applications.entitlements.delete('891', '891', { path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(DiscordAPI.NotFoundError);
  });
  ...
  test('consume: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.applications.entitlements.consume('891', '891', { path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(DiscordAPI.NotFoundError);
  });
});
