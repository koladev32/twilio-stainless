...
describe('resource commands', () => {
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.applications.commands.retrieve('891', '891', { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('update: required and optional params', async () => {
    const response = await client.applications.commands.update('891', [{ name: 'x', id: '891', contexts: [0], default_member_permissions: 0, description: 'description', description_localizations: { foo: 'x' }, dm_permission: true, handler: 0, integration_types: [0], name_localizations: { foo: 'x' }, options: [{ description: 'x', name: 'x', type: 1, description_localizations: { foo: 'x' }, name_localizations: { foo: 'x' }, required: true }], type: 1 }]);
  });
  ...
  test('list: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.applications.commands.list('891', { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(DiscordAPI.NotFoundError);
  });
  ...
  test('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.applications.commands.list('891', { with_localizations: true }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(DiscordAPI.NotFoundError);
  });
  ...
  test('delete: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.applications.commands.delete('891', '891', { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(DiscordAPI.NotFoundError);
  });
});
