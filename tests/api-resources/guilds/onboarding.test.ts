...
describe('resource onboarding', () => {
  test('retrieve', async () => {
    const responsePromise = client.guilds.onboarding.retrieve('891');
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.guilds.onboarding.retrieve('891', { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(DiscordAPI.NotFoundError);
  });

  test('update', async () => {
    const responsePromise = client.guilds.onboarding.update('891', {});
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
