...
describe('resource voiceStates', () => {
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.guilds.voiceStates.retrieve('891', '891', { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('retrieveSelf: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.guilds.voiceStates.retrieveSelf('891', { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(DiscordAPI.NotFoundError);
  });
