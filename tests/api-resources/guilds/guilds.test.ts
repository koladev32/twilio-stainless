...
describe('resource guilds', () => {
  ...
  test('create: required and optional params', async () => {
    const response = await client.guilds.create({
      name: 'xx',
      afk_channel_id: '891',
      afk_timeout: 60,
      channels: [
        {
          name: 'x',
          id: '891',
          available_tags: [{ name: 'name', emoji_id: '891', emoji_name: 'emoji_name', moderated: true }],
          bitrate: 8000,
          default_auto_archive_duration: 60,
          default_forum_layout: 0,
          default_reaction_emoji: { emoji_id: '891', emoji_name: 'emoji_name' },
          default_sort_order: 0,
          default_thread_rate_limit_per_user: 0,
          nsfw: true,
          parent_id: '891',
          permission_overwrites: [{ id: '891', allow: 0, deny: 0, type: 0 }],
          position: 0,
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.guilds.retrieve('891', { path: '/_stainless_unknown_path' })).rejects.toThrow(
  ...
  test('retrieve: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.guilds.retrieve('891', { with_counts: true }, { path: '/_stainless_unknown_path' }),
  ...
  test('delete: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.guilds.delete('891', { path: '/_stainless_unknown_path' })).rejects.toThrow(
      DiscordAPI.NotFoundError,
    );
  });
