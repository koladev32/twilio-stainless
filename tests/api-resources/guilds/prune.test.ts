...
describe('resource prune', () => {
  test('retrieve', async () => {
    const responsePromise = client.guilds.prune.retrieve('891');
  ...
  test('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.guilds.prune.retrieve('891', { path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('retrieve: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.guilds.prune.retrieve('891', { days: 1, include_roles: 'string' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(DiscordAPI.NotFoundError);
  });
});
