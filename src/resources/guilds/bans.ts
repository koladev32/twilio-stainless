...
export class Bans extends APIResource {
  ...
  update(guildId: string, userId: string, body: BanUpdateParams, options?: Core.RequestOptions): Core.APIPromise<void> {
    return this._client.put(`/guilds/${guildId}/bans/${userId}`, { body, ...options, headers: { Accept: '*/*', ...options?.headers } });
  }

  list(guildId: string, query?: BanListParams, options?: Core.RequestOptions): Core.APIPromise<BanListResponse | null>
  list(guildId: string, options?: Core.RequestOptions): Core.APIPromise<BanListResponse | null>
  list(guildId: string, query: BanListParams | Core.RequestOptions = {}, options?: Core.RequestOptions): Core.APIPromise<BanListResponse | null> {
  ...
  delete(guildId: string, userId: string, options?: Core.RequestOptions): Core.APIPromise<void> {
    return this._client.delete(`/guilds/${guildId}/bans/${userId}`, { ...options, headers: { Accept: '*/*', ...options?.headers } });
  }
}
