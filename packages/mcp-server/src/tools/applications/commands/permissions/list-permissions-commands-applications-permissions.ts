...
export const metadata: Metadata = {
  ...
  tags: [],
  httpMethod: 'get',
  httpPath: '/applications/{application_id}/guilds/{guild_id}/commands/permissions',
  operationId: 'list_guild_application_command_permissions',
};

export const tool: Tool = {
  name: 'list_permissions_commands_applications_permissions',
  description:
...
export const handler = async (client: DiscordAPI, args: Record<string, unknown> | undefined) => {
  const { application_id, guild_id, jq_filter, ...body } = args as any;
  return asTextContentResult(
    await maybeFilter(
      jq_filter,
      await client.applications.commands.permissions.listPermissions(application_id, guild_id),
    ),
  );
};
